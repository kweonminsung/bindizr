...
/// Verify that `dns.nsupdate.initial_key` seeds a usable global TSIG key.
#[tokio::test]
#[serial]
async fn the_configured_initial_key_signs_updates_without_the_cli() {
    // Base64 of a 32-byte secret, the shape `tsig-key create` generates.
    let secret = "MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=";
    let app = TestApp::start_with_options(TestAppOptions {
        initial_key: Some(("seeded-key".to_string(), secret.to_string())),
        ..TestAppOptions::default()
    })
    .await;
    let zone_name = app.zone_name("initial-key.example");
    app.create_zone_cli(&zone_name, "3600").await;

    // Global, so it updates the zone without any grant having been made.
    let key = SigningKey {
    ...
    // An unsigned update is still refused: seeding a key does not open the door.
    let refused = send_update(
        app.dns_port(),
        &zone_name,
        &[],
        &[UpdateRr::AddA {
            name: format!("no.{zone_name}"),
            ttl: 300,
            addr: "192.0.2.41".to_string(),
        }],
    )
    .expect("unsigned update");
    assert_eq!(refused, Rcode::REFUSED);
}
