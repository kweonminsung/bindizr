...
pub struct MySqlDnsInstanceRepository {
    pool: Pool<MySql>,
}

impl MySqlDnsInstanceRepository {
...
#[async_trait]
impl DnsInstanceRepository for MySqlDnsInstanceRepository {
    async fn create(&self, mut dns_instance: DnsInstance) -> Result<DnsInstance, DatabaseError> {
        ...
        dns_instance.id = result.last_insert_id() as i32;
