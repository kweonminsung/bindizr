...
impl KeyRepository for PostgresKeyRepository {
    ...
    async fn get_by_id(&self, id: i32) -> Result<Option<Key>, DatabaseError> {
        ...
        let key = sqlx::query_as::<_, Key>(
            "SELECT id, name, key_type, key_algorithm, secret, created_at FROM keys WHERE id = $1",
        )
        .bind(id)
    ...
    async fn get_by_name(&self, name: &str) -> Result<Option<Key>, DatabaseError> {
        ...
        let key = sqlx::query_as::<_, Key>(
            "SELECT id, name, key_type, key_algorithm, secret, created_at FROM keys WHERE name = $1"
        )
        .bind(name)
        .fetch_optional(&mut *conn)
        .await?;
    ...
    async fn update(&self, key: Key) -> Result<Key, DatabaseError> {
        ...
        sqlx::query(
            r#"
            UPDATE keys 
            SET name = $1, key_type = $2, key_algorithm = $3, secret = $4
            WHERE id = $5
            "#,
        )
        .bind(&key.name)
